...
self.addEventListener("push", event => {
  ...
  event.waitUntil((async () => {
    // Im geöffneten, fokussierten Chat reicht die laufende Nachrichtenanzeige.
    // Bei geschlossener oder im Hintergrund befindlicher App bleibt Push aktiv.
    const windows = await self.clients.matchAll({ type: "window", includeUncontrolled: true });
    const appIsFocused = windows.some(client =>
      client.url.startsWith(scope) && client.focused
    );
    if (appIsFocused) return;

    return self.registration.showNotification("📬 Neue Privatnachricht", {
