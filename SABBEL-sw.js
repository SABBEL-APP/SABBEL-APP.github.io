...
self.addEventListener("fetch", event => {
  ...
  event.respondWith((async () => {
    try {
      // ONLINE:
      // Immer zuerst die aktuelle Datei vom Server holen.
      const response = await fetch(request, {
        cache: "no-store"
      });

      // Erfolgreiche Dateien derselben Herkunft
      // als Offline-Version speichern.
      if (
        response &&
        response.ok &&
      ...
      // OFFLINE:
      // Zuletzt erfolgreich geladene Version verwenden.
      const cached =
