...
self.addEventListener("fetch", event => {
  ...
  // Die Netzantwort wird sofort an die App weitergegeben.
  // Das Speichern läuft getrennt im Hintergrund und kann
  // den Android-Startbildschirm daher nicht mehr festhalten.
  ...
  if (request.mode === "navigate") {
    const cachedAfterTimeout = new Promise(resolve => {
      setTimeout(async () => {
        try {
          resolve(await caches.match(request, {
            ignoreSearch: true
          }));
        } catch (error) {
          resolve(null);
    ...
    await self.registration
      .showNotification(
        sender + " hat dich angesabbelt:",
        {
          ...
          // Kleines Symbol in der Android-Statusleiste
          badge: new URL(
            "badge-192.png",
            scope
          ).href,
